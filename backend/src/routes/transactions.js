...
router.put('/:id', [
  ...
  try {
    ...
    if (oldTransaction.type === 'income') {
      runQuery('UPDATE accounts SET balance = balance - ? WHERE id = ? AND user_id = ?', [oldTransaction.amount, oldTransaction.account_id, req.userId]);
    } else if (oldTransaction.type === 'expense') {
      runQuery('UPDATE accounts SET balance = balance + ? WHERE id = ? AND user_id = ?', [oldTransaction.amount, oldTransaction.account_id, req.userId]);
    } else if (oldTransaction.type === 'transfer') {
      runQuery('UPDATE accounts SET balance = balance + ? WHERE id = ? AND user_id = ?', [oldTransaction.amount, oldTransaction.account_id, req.userId]);
      if (oldTransaction.to_account_id) {
        runQuery('UPDATE accounts SET balance = balance - ? WHERE id = ? AND user_id = ?', [oldTransaction.amount, oldTransaction.to_account_id, req.userId]);
      }
    }
    ...
    `, [account_id, to_account_id, category_id, type, description, amount, date, notes, req.params.id, req.userId]);

    const updatedTransaction = getOne('SELECT * FROM transactions WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
    if (updatedTransaction.type === 'income') {
      runQuery('UPDATE accounts SET balance = balance + ? WHERE id = ? AND user_id = ?', [updatedTransaction.amount, updatedTransaction.account_id, req.userId]);
    } else if (updatedTransaction.type === 'expense') {
      runQuery('UPDATE accounts SET balance = balance - ? WHERE id = ? AND user_id = ?', [updatedTransaction.amount, updatedTransaction.account_id, req.userId]);
    } else if (updatedTransaction.type === 'transfer') {
      runQuery('UPDATE accounts SET balance = balance - ? WHERE id = ? AND user_id = ?', [updatedTransaction.amount, updatedTransaction.account_id, req.userId]);
      if (updatedTransaction.to_account_id) {
        runQuery('UPDATE accounts SET balance = balance + ? WHERE id = ? AND user_id = ?', [updatedTransaction.amount, updatedTransaction.to_account_id, req.userId]);
      }
    }
...
router.delete('/:id', (req, res) => {
  try {
    ...
    if (transaction.type === 'income') {
      runQuery('UPDATE accounts SET balance = balance - ? WHERE id = ? AND user_id = ?', [transaction.amount, transaction.account_id, req.userId]);
    } else if (transaction.type === 'expense') {
      runQuery('UPDATE accounts SET balance = balance + ? WHERE id = ? AND user_id = ?', [transaction.amount, transaction.account_id, req.userId]);
    } else if (transaction.type === 'transfer') {
      runQuery('UPDATE accounts SET balance = balance + ? WHERE id = ? AND user_id = ?', [transaction.amount, transaction.account_id, req.userId]);
      if (transaction.to_account_id) {
        runQuery('UPDATE accounts SET balance = balance - ? WHERE id = ? AND user_id = ?', [transaction.amount, transaction.to_account_id, req.userId]);
      }
    }

    runQuery('DELETE FROM transactions WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
