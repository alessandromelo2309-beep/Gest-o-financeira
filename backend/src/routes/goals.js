...
router.put('/:id', (req, res) => {
  try {
    ...
    runQuery(
      'UPDATE goals SET name=COALESCE(?,name), icon=COALESCE(?,icon), target_amount=COALESCE(?,target_amount), current_amount=COALESCE(?,current_amount), monthly_contribution=COALESCE(?,monthly_contribution), deadline=COALESCE(?,deadline), color=COALESCE(?,color), status=COALESCE(?,status) WHERE id=? AND user_id=?',
      [name, icon, target_amount, current_amount, monthly_contribution, deadline, color, status, req.params.id, req.userId]
    );
    res.json(getOne('SELECT * FROM goals WHERE id = ? AND user_id = ?', [req.params.id, req.userId]));
  } catch (err) {
...
router.post('/:id/add', [
  ...
  try {
    ...
    runQuery(
      ...
      [newAmount, completed ? 'completed' : goal.status, req.params.id, req.userId]
    );
    res.json(getOne('SELECT * FROM goals WHERE id = ? AND user_id = ?', [req.params.id, req.userId]));
  } catch (err) {
...
router.delete('/:id', (req, res) => {
  try {
    const goal = getOne('SELECT * FROM goals WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
    if (!goal) return res.status(404).json({ error: 'Meta não encontrada' });
    runQuery('DELETE FROM goals WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
