...
function getFinancialData(userId) {
  ...
  const totalBalance = getOne('SELECT COALESCE(SUM(balance), 0) as t FROM accounts WHERE user_id = ?', [userId])?.t || 0;
  const accounts = getAll('SELECT name, type, balance FROM accounts WHERE user_id = ?', [userId]);
  const categories = getAll('SELECT name, type FROM categories WHERE user_id = ?', [userId]);
  const cardCount = getOne('SELECT COUNT(*) as c FROM credit_cards WHERE user_id = ?', [userId])?.c || 0;
  const cards = getAll('SELECT name, limit_amount FROM credit_cards WHERE user_id = ?', [userId]);
  const goalCount = getOne("SELECT COUNT(*) as c FROM goals WHERE user_id = ? AND status = 'active'", [userId])?.c || 0;
  const goals = getAll("SELECT name, target_amount, current_amount FROM goals WHERE user_id = ? AND status = 'active'", [userId]);
  const budgetCount = getOne('SELECT COUNT(*) as c FROM budgets WHERE user_id = ?', [userId])?.c || 0;

  const currIncome = getOne("SELECT COALESCE(SUM(amount), 0) as t FROM transactions WHERE user_id = ? AND type = 'income' AND strftime('%m', date) = ? AND strftime('%Y', date) = ?", [userId, cm, cy])?.t || 0;
  const currExpenses = getOne("SELECT COALESCE(SUM(amount), 0) as t FROM transactions WHERE user_id = ? AND type = 'expense' AND strftime('%m', date) = ? AND strftime('%Y', date) = ?", [userId, cm, cy])?.t || 0;
  const prevIncome = getOne("SELECT COALESCE(SUM(amount), 0) as t FROM transactions WHERE user_id = ? AND type = 'income' AND strftime('%m', date) = ? AND strftime('%Y', date) = ?", [userId, pm, py.toString()])?.t || 0;
  const prevExpenses = getOne("SELECT COALESCE(SUM(amount), 0) as t FROM transactions WHERE user_id = ? AND type = 'expense' AND strftime('%m', date) = ? AND strftime('%Y', date) = ?", [userId, pm, py.toString()])?.t || 0;
  ...
  const topCats = getAll(`
    ...
    WHERE c.user_id = ? AND c.type = 'expense' GROUP BY c.id ORDER BY total DESC LIMIT 5
  `, [cm, cy, userId]);

  const recentTx = getAll(`
  ...
  return {
    totalBalance, accounts, categories, cardCount, cards, goalCount, goals, budgetCount,
    currIncome, currExpenses, currResult: currIncome - currExpenses,
    prevIncome, prevExpenses, prevResult: prevIncome - prevExpenses,
    topCats, recentTx, cm, cy,
    savingsRate: currIncome > 0 ? ((currIncome - currExpenses) / currIncome * 100).toFixed(1) : 0,
    expChange: prevExpenses > 0 ? (((currExpenses - prevExpenses) / prevExpenses) * 100).toFixed(1) : null,
    incChange: prevIncome > 0 ? (((currIncome - prevIncome) / prevIncome) * 100).toFixed(1) : null,
  };
}
  ...
  switch (intent) {
    ...
    case 'joke': {
      const jokes = [
        ...
        'O que o matemático disse para o outro? "Não sou你的raiz, mas posso elevar seu humor!" 😂',
      ];
...
router.post('/', (req, res) => {
  try {
    ...
    const data = getFinancialData(req.userId);
...
router.get('/history', (req, res) => {
  ...
  res.json(hist.map(h => ({ role: h.role, text: h.text })));
});
